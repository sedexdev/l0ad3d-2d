...
--[[
    ...
    Params:
        ...
        playerX:         table - Player x to use for the relative positioning of the Boss
        playerX:         table - Player y to use for the relative positioning of the Boss
        collisionSystem: table - collisionSystem object
        enemySystem:     table - EnemySystem object
    Returns:
        nil
]]
function BossRushingState:init(area, boss, playerX, playerY, collisionSystem, enemySystem)
    self.area = area
    self.boss = boss
...
function BossRushingState:update(dt)
    ...
    if (self.playerX < self.boss.x) and (self.playerY < self.boss.y) then
    ...
    if (self.playerX < self.boss.x) and (math.abs(self.boss.y - self.playerY) <= ENTITY_AXIS_PROXIMITY) then
    ...
    if (math.abs(self.boss.x - self.playerX) <= ENTITY_AXIS_PROXIMITY) and (self.playerY < self.boss.y) then
